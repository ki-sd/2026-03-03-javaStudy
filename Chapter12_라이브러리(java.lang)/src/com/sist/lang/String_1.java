 ...
 *       = 검색
 *         ***startsWith() : 시작 문자열 같은지 => 쿠키에 저장된 데이터
 *         endsWith() : 끝 문자열 같은지 => 확장자 => 다운로드 / 파일 첨부
 ...
 *         **length() : 문자갯수
 *         **trim() : 공백제거
 *         ***substring() : 원하는 위치에서 문자를 자르기
 *         ***indexOf() : 문자, 문자열 위치 찾기 (앞에서)
 *         ***lastIndexOf() :뒤에서
 *       = 모든 데이터형을 문자열로 변환
 *         => static => ***valueOf()
 *      ----------------------------------------------------
 *      불변 => 변경되지 않음 => 변환 => String 리턴형
 *      
 *      String s="Hello";
